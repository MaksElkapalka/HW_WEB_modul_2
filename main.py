...
style = Style.from_dict(
    {  # містить інформацію про те, які стилі повинні застосовуватися до різних елементів інтерфейсу
...
class Record:
    ...
    def __str__(self):
        ...
        emails_str = (
            ", ".join(str(e.value) for e in self.email) if self.email else "None"
        )
...
class AddressBook(UserDict):
    ...
    def search(
        self, value=None
    ):  # пошук за за кількома цифрами номера телефону або літерами імені
        if value:
            data_match = str()
            value = value.lower()
...
class Notes(UserList):
    ...
    def sort_by_date_reverse(self):
        if self:
            ...
            return "\n--------------\n\n".join(
                str(note) for note in sorted_by_date_reverse
            )
        else:
            return "Your notebook is empty"
    ...
    if record:
        ...
        while True:
            ...
            if choice == "1":
                ...
                new_birthday = input("Enter the new birthday in format dd-mm-yyyy: ")
...
def edit_note():
    ...
    while True:
        ...
        if choice == "1":
            ...
            new_tags = input("Enter the new tags separated by commas: ").split(",")
...
def main():
    ...
    while True:
        try:
            user_input = prompt(
                "Enter command: ", completer=completer, style=style
            )  # input('>>> ')
